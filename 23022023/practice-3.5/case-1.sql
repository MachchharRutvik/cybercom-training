...
VALUES
  ...
  ('Gizmo', 19.99, 'A large gizmo'),
  ('Thingamajig', 5.99, 'A thingamajig'),
  ('Doohickey', 8.99, 'A doohickey'),
  ('Whatchamacallit', 14.99, 'A whatchamacallit');

INSERT INTO products (name, unit_price, description)
VALUES  ('Doohickey', 55, 'A doohickey');
...
select c.first_name,c.last_name,c.email,count(o.customer_id) as total_orders
from customers c
join orders o 
...
select p.name,count(o.product_id) as number_of_orders,sum(p.unit_price) as total_revenue
from products p
join orders o
...
-- orders.
SELECT 
    c.first_name, c.last_name, c.email, p.name
FROM
    customers c
        JOIN
...
from orders
group by customer_id
having count(customer_id)>=1) and p.unit_price > 50;
...
select p.name,count(o.product_id) as total_orders_of_product
from products p 
join orders o
on p.id = o.product_id
group by o.product_id
having total_orders_of_product > 1;